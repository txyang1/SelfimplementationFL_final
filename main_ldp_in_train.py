...
from models.update1 import LocalUpdate3
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):  # 循环进行多个训练轮次 default=10 t= 1,2,3....
        ...
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)  # S_t
        for idx in idxs_users:  # ：遍历选定的客户端 k k=1,...,10
            # ：初始化本地更新对象，传入参数、训练数据集和客户端索引
            #optimizer = torch.optim.SGD(copy.deepcopy(net_glob.parameters()), lr=args.lr, momentum=args.momentum)
            local = LocalUpdate3(args=args, dataset=dataset_train,
                                idxs=dict_users[idx])
            loss,grad,nk = local.train(net=copy.deepcopy(net_glob))
            loss_locals.append(copy.deepcopy(loss))
            #wait until all the gradients
            grad_locals.append(copy.deepcopy(grad))
            noisy_grad.append(copy.deepcopy(grad))#add noise into gradient
            nk_list.append(copy.deepcopy(nk))



        '''#torch.save(grad_locals, 'save/grad_locals_epoch_5_att_epsilon0.1_407.pth')'''

        if args.attack == 'xie':
            if iter >= args.addtime:
                n = len(noisy_grad)
                if n > 2 * args.malicious+2:
                    selected_indices = random.sample(range(len(noisy_grad)), args.malicious)
                    print(selected_indices)
                    non_selected_indices = [i for i in range(m) if i not in selected_indices]
                    non_byzantine_grad_locals = [noisy_grad[idx] for idx in non_selected_indices]
                    ...
                    for idx in selected_indices:
                        noisy_grad[idx] = grad_correct_avg
        ...
        if args.Agg == 'Krum':
           ...
           grad_glob, min_index = Krum(noisy_grad, args.malicious)
            ...
            grad_glob = fedavg(noisy_grad,nk_list)
            # average the noisy_grad
            #grad_glob = Avg(noisy_grad)
            #grad_glob = Avg(clipped_grad)


        with torch.no_grad():
            for name in w_glob.keys():
                #if name in grad_glob:
                    w_glob[name] = w_glob[name]-0.01 * grad_glob[name]
                #if name in avg_grad:
                    #w_glob[name] = w_glob[name] - 0.01 * avg_grad[name]
    ...
    with open('./result/z_sigma_ldp_grad_in_train_{}_{}_attack_{}_epsilon_{}_addtime_{}_malicious_{}_epoch_{}_users_{}_sigma_{}_C_{}.txt'.format(args.dataset,args.Agg, args.attack,args.epsilon,args.addtime,args.malicious,args.epochs,args.frac,args.sigma,args.C), 'w') as f:
        f.write("Epoch\tTest Accuracy\tMin Index\tSelected Indices\n")
        for epoch in range(args.epochs):
            #if epoch < args.addtime:
             #   f.write("{:d}\t{:.3f}\tNA\tNA\n".format(epoch, test_accuracy[epoch]))
